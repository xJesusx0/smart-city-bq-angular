...
@Component({
  ...
  template: `
    <div class="p-6 max-w-7xl mx-auto flex flex-col gap-6">
      <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p class="text-muted-foreground">
            Estado en tiempo real de los controladores de semáforos.
          </p>
        </div>

        <div
          class="flex items-center gap-2 text-sm font-medium bg-secondary/50 px-4 py-2 rounded-full border"
        >
          @if (intersectionsQuery.isFetching()) {
            ...
            <span class="text-muted-foreground"
              >Última actualización: {{ lastUpdate() | date: 'HH:mm:ss' }}</span
            >
          }
        </div>
      </header>
      ...
      @if (intersectionsQuery.isError()) {
        <div
          class="flex flex-col items-center justify-center p-12 bg-destructive/5 text-destructive rounded-xl border border-destructive/20 shadow-sm"
        >
          <hlm-icon name="lucideAlertCircle" class="h-16 w-16 mb-4 opacity-50" />
          <p class="text-xl font-bold">Error al cargar datos</p>
          <p class="text-muted-foreground mb-6">
            No pudimos obtener la información de las intersecciones en este momento.
          </p>
          <button
        ...
        <main class="transition-all duration-300">
          @if (!selectedIntersection()) {
            <section
              hlmCard
              class="overflow-hidden border-none shadow-xl bg-card/50 backdrop-blur-sm"
            >
  ...
  styles: `
    :host {
      ...
      background-image:
        radial-gradient(at 0% 0%, hsla(var(--primary-foreground), 0.05) 0px, transparent 50%),
