            ...
            @for (step of CYCLE_STATES; track $index; let last = $last) {
              <div class="flex flex-col items-center min-w-0">
                ...
                <div
                  ...
                  [class.ring-2]="step.estado === simulatedData()?.estado && activeStepIndex() === $index"
                  [class.ring-offset-1]="step.estado === simulatedData()?.estado && activeStepIndex() === $index"
                  [ngClass]="getStateMapNodeClass(step.estado, step.estado === simulatedData()?.estado && activeStepIndex() === $index)"
                  [class.scale-105]="step.estado === simulatedData()?.estado && activeStepIndex() === $index"
                >
                  <!-- Mini semaphores -->
                  ...
                  <!-- Label -->
                  <span class="text-[9px] font-mono font-semibold text-center leading-tight whitespace-nowrap">
                    {{ step.label }}
                  </span>
                  <!-- Duration -->
                ...
                <!-- Current indicator -->
                <div class="flex flex-col items-center mt-1 gap-0.5 transition-opacity duration-300"
                     [class.opacity-0]="!(step.estado === simulatedData()?.estado && activeStepIndex() === $index)">
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col gap-2 flex-1">
                  <div>
                    ...
                    <p class="font-mono text-sm font-semibold">{{ simulatedData()?.ciclo_restante_s }}s</p>
                  </div>
            ...
            <div class="border rounded-md p-4">
              ...
              <div class="mb-4">
                ...
                <div class="flex h-6 rounded-sm overflow-hidden border">
                  ...
                  >
                    <span class="text-[8px] text-white font-semibold truncate px-0.5">{{ simulatedData()?.semaforo1_verde }}s</span>
                  </div>
                  <div
                  ...
                  >
                    <span class="text-[8px] text-white font-semibold truncate px-0.5">{{ simulatedData()?.semaforo2_verde }}s</span>
                  </div>
                  <div
                    class="bg-red-800/80 flex items-center justify-center"
                    [style.flex]="simulatedData()?.all_red_time"
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="border p-2.5 rounded-sm">
                  ...
                  <p class="font-mono text-xl font-bold text-emerald-600 leading-tight mt-0.5">
                    {{ simulatedData()?.semaforo1_verde }}<span class="text-xs font-normal text-muted-foreground">s</span>
                  </p>
                ...
                <div class="border p-2.5 rounded-sm">
                  ...
                  <p class="font-mono text-xl font-bold text-emerald-600 leading-tight mt-0.5">
                    {{ simulatedData()?.semaforo2_verde }}<span class="text-xs font-normal text-muted-foreground">s</span>
                  </p>
            ...
            <div class="border rounded-md p-4 flex flex-col gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mt-2 flex items-center gap-1.5">
                    <div class="h-1.5 rounded-full bg-emerald-500/30 flex-1 overflow-hidden">
                      <div class="h-full bg-emerald-500 rounded-full" [style.width.%]="nextS1Percentage()"></div>
                    </div>
                    <span class="text-[9px] text-muted-foreground font-mono">{{ nextS1Percentage() }}%</span>
                  </div>
                </div>
                <span class="text-xs text-muted-foreground font-mono shrink-0">{{ simulatedData()?.next_semaforo1 }}s</span>
              </div>
              ...
              <div class="border rounded-sm p-3 flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mt-2 flex items-center gap-1.5">
                    <div class="h-1.5 rounded-full bg-emerald-500/30 flex-1 overflow-hidden">
                      <div class="h-full bg-emerald-500 rounded-full" [style.width.%]="nextS2Percentage()"></div>
                    </div>
                    <span class="text-[9px] text-muted-foreground font-mono">{{ nextS2Percentage() }}%</span>
                  </div>
                </div>
                <span class="text-xs text-muted-foreground font-mono shrink-0">{{ simulatedData()?.next_semaforo2 }}s</span>
              </div>
              ...
              <div class="border-t pt-3">
                ...
                <div class="flex flex-col gap-1.5">
                  <div class="flex items-center gap-2 text-[10px]">
                    ...
                    <span class="font-mono font-semibold">{{ simulatedData()?.semaforo1_verde }}s</span>
                    <span class="text-muted-foreground">→</span>
                    <span class="font-mono font-semibold">{{ simulatedData()?.next_semaforo1 }}s</span>
                    <span
                      class="ml-auto text-[9px] font-semibold"
                      [class.text-emerald-600]="(simulatedData()?.next_semaforo1 || 0) > (simulatedData()?.semaforo1_verde || 0)"
                      [class.text-red-500]="(simulatedData()?.next_semaforo1 || 0) < (simulatedData()?.semaforo1_verde || 0)"
                      [class.text-muted-foreground]="(simulatedData()?.next_semaforo1 || 0) === (simulatedData()?.semaforo1_verde || 0)"
                    >{{ s1Delta() }}</span>
                  </div>
                  <div class="flex items-center gap-2 text-[10px]">
                    <span class="text-muted-foreground w-16 shrink-0">S2</span>
                    <span class="font-mono font-semibold">{{ simulatedData()?.semaforo2_verde }}s</span>
                    <span class="text-muted-foreground">→</span>
                    <span class="font-mono font-semibold">{{ simulatedData()?.next_semaforo2 }}s</span>
                    <span
                      class="ml-auto text-[9px] font-semibold"
                      [class.text-emerald-600]="(simulatedData()?.next_semaforo2 || 0) > (simulatedData()?.semaforo2_verde || 0)"
                      [class.text-red-500]="(simulatedData()?.next_semaforo2 || 0) < (simulatedData()?.semaforo2_verde || 0)"
                      [class.text-muted-foreground]="(simulatedData()?.next_semaforo2 || 0) === (simulatedData()?.semaforo2_verde || 0)"
                    >{{ s2Delta() }}</span>
                  </div>
                </div>
              </div>
...
export class IntersectionDetailComponent {
  ...
  simulatedData = computed(() => {
    ...
    let totalCycle = (raw.semaforo1_verde || 0) + (raw.semaforo2_verde || 0) + (raw.all_red_time || 0) * 2;
  ...
  stateDescription = computed(() => {
    ...
    switch (estado) {
      case 'S1_VERDE':           return `${calleA} tiene paso libre`;
      case 'S2_VERDE':           return `${calleB} tiene paso libre`;
      case 'S1_AMARILLO':        return `${calleA} preparando cierre`;
      case 'S2_AMARILLO':        return `${calleB} preparando cierre`;
      case 'S1_ROJO':            return `${calleA} detenida`;
      case 'S2_ROJO':            return `${calleB} detenida`;
      case 'S1_ROJO_AMARILLO':   return `${calleA} preparando apertura`;
      case 'S2_ROJO_AMARILLO':   return `${calleB} preparando apertura`;
  ...
  getStateBadgeClass(estado?: string): string {
    switch (estado) {
      case 'S1_VERDE':
      case 'S2_VERDE':           return 'bg-emerald-50 text-emerald-700 border-emerald-200';
      case 'S1_AMARILLO':
      case 'S2_AMARILLO':        return 'bg-amber-50 text-amber-700 border-amber-200';
      case 'S1_ROJO':
      case 'S2_ROJO':            return 'bg-red-50 text-red-700 border-red-200';
      case 'S1_ROJO_AMARILLO':
      case 'S2_ROJO_AMARILLO':   return 'bg-orange-50 text-orange-700 border-orange-200';
      case 'ALL_RED':            return 'bg-red-100 text-red-800 border-red-300';
  ...
  isYellow(semaphore: 'S1' | 'S2'): boolean {
    ...
    if (!estado) return false;
    return semaphore === 'S1'
      ? estado.includes('S1_AMARILLO')
      : estado.includes('S2_AMARILLO');
  }
